...
#define ENC_MAX_OVERFLOW_NB     (1)
    ...
    // 设置滤波
    EQEP_setFilterRange(EQEPx, 10);
    EQEP_enableFilter(EQEPx);

    // 中断使能
    Interrupt_register(EQEP_INTR1_IRQn, ENCODER_IRQHandler);
    Interrupt_enable(EQEP_INTR1_IRQn);

    // 模块使能
    EQEP_enableCapture(EQEPx);
    EQEP_enableModule(EQEPx);
...
__WEAK int16_t ENC_CalcAngle(ENCODER_Handle_t * pHandle)
{
    ...
    wtemp1 = (int32_t)(EQEP_getPosition(pHandle->EQEPx) & 0xffff) *
...
#endif
    CntCapture =  EQEP_getPosition(EQEPx);
    OverflowCntSample = pHandle->TimerOverflowNb;
    pHandle->TimerOverflowNb = 0;
    directionSample =  EQEP_getDirection(EQEPx);
#ifdef TIM_CNT_UIFCPY
...
__WEAK void ENC_SetMecAngle(ENCODER_Handle_t * pHandle, int16_t hMecAngle)
{
    ...
    EQEPx->PCNT_CFG = (uint16_t)(hAngleCounts);
