...
{
  ADC_Type * ADCx_1;             /*!< It contains the pointer to the first ADC
                                        used for current reading. */
  ADC_Type * ADCx_2;             /*!< It contains the pointer to the second ADC
                                        used for current reading. */
  SRPWM_Type *SRPWMx;                   /*!< It contains the pointer to the timer
                                            used for PWM generation. It must
                                            equal to TIM1 if M1, to TIM8 otherwise */
  GPIO_Type * pwm_en_u_port;     /*!< phase u enable driver signal GPIO port */
  GPIO_Type * pwm_en_v_port;     /*!< phase v enable driver signal GPIO port */
  GPIO_Type * pwm_en_w_port;     /*!< phase w enable driver signal GPIO port */
  GPIO_PinNumber      pwm_en_u_pin;       /*!< phase u enable driver signal pin */
  GPIO_PinNumber      pwm_en_v_pin;       /*!< phase v enable driver signal pin */
  GPIO_PinNumber      pwm_en_w_pin;       /*!< phase w enable driver signal pin */

  const volatile uint32_t  *ADCDataReg1[6];
  const volatile uint32_t  *ADCDataReg2[6];
  uint32_t ADCConfig1[6];
  uint32_t ADCConfig2[6];
  uint32_t ADCConfig_VC1[6];
  uint32_t ADCConfig_VC2[6];
