/**
 * @file mc_config.c
 * @brief 电机控制对象及控制器参数的静态实例化。
 *
 * 中文解读：这里把速度PI、Id/Iq电流PI、开环启动曲线、STO-PLL观测器、母线/温度传感器、
 * PWM电流采样对象和状态机组合成Motor 1。结构体成员多由参数头文件宏展开；算法结构通常
 * 不改，调参应优先修改 drive_parameters.h、pmsm_motor_parameters.h 等参数源。
 */
/*
...
/**
  * @brief  当前接口：处理并更新速度、PI/PID调节器、电机、参数和增益，保持对象状态和控制流程一致。
  */
...
/**
  * @brief  当前接口：处理并更新PI/PID调节器、电机、参数和增益，保持对象状态和控制流程一致。
  */
...
/**
  * @brief  当前接口：处理并更新速度、转矩、PI/PID调节器、电机、参数和增益，保持对象状态和控制流程一致。
  */
...
PWMC_R3_2_Handle_t PWM_Handle_M1=
{
  ...
  .PhaseBOffset = 0,
  .PhaseCOffset = 0,
  .ADCTriggerEdge = SRPWM_OUTEVT_SYNC_CMPA_UP_EVT,  
  .CalibSector = SECTOR_1,
  .pParams_str = &R3_2_ParamsM1
};

/**
  * @brief  当前接口：处理并更新速度、位置和角度、电机、参数和增益，保持对象状态和控制流程一致。
  */
...
/**
  * @brief  当前接口：处理并更新速度、位置和角度、状态机状态、观测器、反电势和PLL、电机、参数和增益，保持对象状态和控制流程一致。
  */
...
/**
  * @brief  当前接口：处理并更新温度、电机、参数和增益，保持对象状态和控制流程一致。
  */
...
/**
  * @brief  当前接口：处理并更新斜坡、电机，保持对象状态和控制流程一致。
  */
