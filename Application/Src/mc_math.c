/**
 * @file mc_math.c
 * @brief FOC 定点数学运算与坐标变换。
 *
 * 中文解读：Clark把三相静止量化为alpha-beta，Park再按电角度旋转为d-q；d轴对应磁链、
 * q轴对应转矩。反变换把PI输出电压还原到静止坐标供SVPWM使用。定点饱和和缩放用于避免
 * 溢出，不应随意改成浮点或删除限幅。
 */
/*
...
#define divSQRT_3 (int32_t)0x49E6    /* 中文说明：处理并更新相关数据和控制状态。 */
...
#endif
/**
  * @brief  MCM_Clarke：处理并更新串口通信帧，保持对象状态和控制流程一致。
...
#endif
/**
  * @brief  MCM_Park：处理并更新位置和角度、串口通信帧，保持对象状态和控制流程一致。
  * @param  Input:  输入参数，提供该接口所需的对象句柄或控制数据。
...
/**
  * @brief  MCM_PhaseComputation：检查并判断位置和角度、观测器、反电势和PLL、串口通信帧，保持对象状态和控制流程一致。
  * @param  wBemf_alfa_est  输入参数，提供该接口所需的对象句柄或控制数据。
  * @retval 接口执行结果、状态标志或计算值；具体类型由函数声明限定。
  */
